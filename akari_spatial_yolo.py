...
def draw_bird_frame(frame, x, z, id=None):
    ...
    if id is not None:
        ...
        cv2.circle(
            frame, (pointX, pointY), 2, idColors[id], thickness=5, lineType=8, shift=0
        )
    else:
        cv2.circle(
            frame, (pointX, pointY), 2, (0, 255, 0), thickness=5, lineType=8, shift=0
        )
            ...
            if msgs is not None:
                ...
                for detection in detections:
                    ...
                    if detection.spatialCoordinates.z != 0:
                        text.putText(
                            display,
                            "X: {:.2f} m".format(float(converted_pos[0]) / 1000),
                            (x2 + 10, y1 + 80),
                        )
                        text.putText(
                            display,
                            "Y: {:.2f} m".format(float(converted_pos[1]) / 1000),
                            (x2 + 10, y1 + 110),
                        )
                        text.putText(
                            display,
                            "Z: {:.2f} m".format(float(converted_pos[2]) / 1000),
                            (x2 + 10, y1 + 140),
                        )
                    draw_bird_frame(
                        birds,
